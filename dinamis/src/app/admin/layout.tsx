...
function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: "The Forge", href: "/admin", icon: LayoutIcon },
    { name: "Perkamen Lore", href: "/admin/berita", icon: NewspaperIcon },
    { name: "Katalog Armory", href: "/admin/layanan", icon: SettingsIcon },
    { name: "Surat Gagak Masuk", href: "/admin/kontak", icon: MessageIcon },
  ];
    ...
    <div className="admin-root">
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? "admin-sidebar--open" : ""}`}>
        <div className="admin-sidebar-logo">
          <HexIcon />
          <span>Iron & <span style={{ color: "#c9a84c" }}>Blade</span></span>
        ...
        <div className="admin-sidebar-footer">
          <div className="admin-user-row">
            ...
            <div className="admin-user-info">
              <div className="admin-user-name">{session?.user?.name || "Master Aldric"}</div>
              <div className="admin-user-role">The Grandmaster</div>
            </div>
          </div>
          <button className="admin-logout-btn" onClick={() => signOut({ callbackUrl: "/login" })}>
            <LogoutIcon /> Kunci Bengkel
          </button>
        </div>
      </aside>
