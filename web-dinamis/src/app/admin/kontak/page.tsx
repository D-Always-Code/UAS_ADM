  ...
  return (
    <div>
      <div className="admin-page-header">
        <div>
          <div className="admin-page-title">Surat Gagak Masuk</div>
          <div className="admin-page-subtitle">
            {unread > 0
              ? <span><strong style={{ color: "#b91c1c" }}>{unread} surat baru</strong> belum dibaca dari {kontak.length} total surat</span>
              : `${kontak.length} surat — semua sudah dibaca`
            }
          </div>
        </div>
      </div>
        ...
        <table className="admin-table">
          <thead>
            <tr>
              ...
              <th>Nama Ksatria / Pengirim</th>
              <th>Jenis Pesanan & Isi Surat</th>
            ...
            {kontak.length === 0 ? (
              <tr>
                <td colSpan={5} style={{ textAlign: "center", padding: "48px", color: "#94a3b8" }}>
                  Belum ada surat gagak yang masuk.
                </td>
              </tr>
            ) : kontak.map((item: any) => (
              <tr key={item.id} style={{ background: item.is_read ? "transparent" : "rgba(239,246,255,0.5)" }}>
                <td style={{ verticalAlign: "middle" }}>
                  {!item.is_read && (
                    <span style={{ display: "block", width: "8px", height: "8px", borderRadius: "50%", background: "#b91c1c" }} />
                  )}
                </td>
